-- Spotify Integration Tables for DISBA Music (FIXED VERSION)
...
-- Table untuk commission tracking (untuk admin dashboard)
...
-- Enable RLS untuk Spotify tables
...
DROP POLICY IF EXISTS "Users can view their own spotify distributions" ON public.spotify_distributions;
DROP POLICY IF EXISTS "Users can view their spotify analytics" ON public.spotify_analytics;
DROP POLICY IF EXISTS "Users can view their artist commissions" ON public.artist_commissions;
DROP POLICY IF EXISTS "Admins can view all commissions" ON public.admin_commissions;

-- Create RLS Policies (FIXED - proper casting for auth.uid text)
CREATE POLICY "Users can view their own spotify distributions" ON public.spotify_distributions
...
CREATE POLICY "Admins can view all commissions" ON public.admin_commissions
FOR SELECT USING (
    EXISTS (
